...
public class FoodMenuController {
    ...
    @FXML
    private void updateMenuItem(ActionEvent event) {
        MenuItem selectedItem = orderTable.getSelectionModel().getSelectedItem();
        if (selectedItem == null) {
            Alert alert = new Alert(Alert.AlertType.WARNING);
            alert.setTitle("No Selection");
            alert.setHeaderText(null);
            alert.setContentText("Please select a menu item to update.");
            alert.showAndWait();
            return;
        }
        // Prompt for new name
        ...
        double newPrice;
        try {
            newPrice = Double.parseDouble(priceResult.get());
        ...
        try (Connection conn = MySQLConnection.getConnection()) {
            String sql = "UPDATE menu_items SET name = ?, price = ? WHERE name = ?";
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
                stmt.setString(1, newName);
                stmt.setDouble(2, newPrice);
                stmt.setString(3, selectedItem.getName());
                int rows = stmt.executeUpdate();
                if (rows > 0) {
                    selectedItem = new MenuItem(newName, selectedItem.getQuantity(), newPrice);
                    int idx = orderTable.getItems().indexOf(orderTable.getSelectionModel().getSelectedItem());
                    orderTable.getItems().set(idx, selectedItem);
                    updateTotal();
                    Alert alert = new Alert(Alert.AlertType.INFORMATION);
                    alert.setTitle("Success");
                    alert.setHeaderText(null);
                    alert.setContentText("Menu item updated successfully.");
                    alert.showAndWait();
                } else {
                    Alert alert = new Alert(Alert.AlertType.ERROR);
                    alert.setTitle("Update Failed");
                    alert.setHeaderText(null);
                    alert.setContentText("Menu item not found in database.");
                    alert.showAndWait();
                }
            }
            ...
            alert.setHeaderText(null);
            alert.setContentText("Failed to update menu item: " + e.getMessage());
            alert.showAndWait();
        }
    }
}